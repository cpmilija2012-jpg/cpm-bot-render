...
BOT_TOKEN = "8800278295:AAG3FNPk86pAt7wP-UOi9I9l4emUbF4Ktd4"
OWNER_ID  = 8884756222
...
class T:
    ...
    def dashboard(record, email, uid):
        ...
        cars   = len(record.get("boughtFsos",[]))
...
class K:
    ...
    def cars():
        return InlineKeyboardMarkup(inline_keyboard=[
            ...
            [InlineKeyboardButton(text="🛒 Buy Car by ID", callback_data="car_buy_id")],
...
async def cb_back_home(cb, state):
    await state.clear()
...
async def cb_cars_menu(cb):
    if not nuker.get_token(cb.from_user.id): await cb.answer("✗ Sign in first!", show_alert=True); return
    await cb.message.edit_text(f"{B}\n  🚗  𝗖𝗔𝗥𝗦\n{B}\n\n  Manage your cars:", reply_markup=K.cars())
    await cb.answer()


@rt.callback_query(F.data.in_(set(FEAT_MAP.keys())))
async def cb_feat(cb):
    uid       = cb.from_user.id
...
async def cb_car_unlock_all(cb):
    uid = cb.from_user.id
    ...
    if r.get("ok"):
        ...
        save_store(STORE); update_daily_stats("unlocks")
    await result_msg(m, r.get("ok"),
        "𝗔𝗟𝗟 𝗖𝗔𝗥𝗦 𝗨𝗡𝗟𝗢𝗖𝗞𝗘𝗗" if r.get("ok") else "𝗙𝗔𝗜𝗟𝗘𝗗",
        f"🚗 {len(CAR_IDS)} cars" if r.get("ok") else r.get("message",""), K.cars())


@rt.callback_query(F.data == "car_buy_id")
async def cb_car_buy_id(cb, state):
    if not nuker.get_token(cb.from_user.id): await cb.answer("✗ Sign in first!", show_alert=True); return
    await state.set_state(SCars.car_id)
    await cb.message.edit_text(hdr("🛒","𝗕𝗨𝗬 𝗖𝗔𝗥")+"\n\n  Enter Car ID:", reply_markup=K.cancel())
    await cb.answer()


@rt.message(SCars.car_id)
async def p_car_id(msg, state):
    try:
        car_id = int(msg.text.strip())
    except ValueError:
        await msg.answer("  ✗ Invalid Car ID! Must be a number.", reply_markup=K.cancel()); return
    await state.clear()
    ld = await msg.answer(f"  ⏳ Buying car {car_id}...")
    r  = await nuker.buy_car(msg.from_user.id, car_id)
    await result_msg(ld, r.get("ok"),
        "𝗖𝗔𝗥 𝗔𝗗𝗗𝗘𝗗" if r.get("ok") else "𝗙𝗔𝗜𝗟𝗘𝗗",
        f"🚗 Car ID {car_id}" if r.get("ok") else r.get("message",""), K.cars())


@rt.callback_query(F.data == "f_all")
async def cb_feat_all(cb):
    uid = cb.from_user.id
    if not nuker.get_token(uid): await cb.answer("✗ Sign in first!", show_alert=True); return
    await cb.answer()
...
async def expire_bulkadd_prompt(user_id, chat_id, state):
...
async def cmd_bulkadd(msg, state):
    uid = msg.from_user.id
...
async def cb_admin_menu(cb, state):
    await state.clear()
    uid = cb.from_user.id
...
async def cb_a_bulkadd(cb, state):
    if not has_admin(cb.from_user.id,"admin"): await cb.answer("✗",show_alert=True); return
    await state.set_state(SAdmin.bulkadd)
...
async def cb_a_adduser(cb, state):
    if not has_admin(cb.from_user.id,"admin"): await cb.answer("✗",show_alert=True); return
...
async def cb_a_expiry(cb, state):
    if not has_admin(cb.from_user.id,"admin"): await cb.answer("✗",show_alert=True); return
...
async def p_expiry_id(msg, state):
    try: uid = int(msg.text.strip())
    except: await msg.answer("  ✗ Invalid ID."); return
...
async def p_expiry_dy(msg, state):
...
async def cb_a_profile(cb, state):
    if not has_admin(cb.from_user.id,"admin"): await cb.answer("✗",show_alert=True); return
...
async def p_profile_id(msg, state):
    try: uid = int(msg.text.strip())
    except: await msg.answer("  ✗ Invalid ID."); return
    await state.clear()
...
async def cb_a_addvip(cb, state):
...
async def p_upload_photo(msg, state):
...
async def cb_bcast_text(cb, state):
    if not has_admin(cb.from_user.id,"superadmin"): await cb.answer("✗",show_alert=True); return
...
async def cb_bcast_vip(cb, state):
    if not has_admin(cb.from_user.id,"superadmin"): await cb.answer("✗",show_alert=True); return
...
async def cb_bcast_photo(cb, state):
    if not has_admin(cb.from_user.id,"superadmin"): await cb.answer("✗",show_alert=True); return
    await state.update_data(bcast_target="all")
...
async def p_bcast_photo_file(msg, state):
...
async def p_bcast_photo_url(msg, state):
...
async def p_bcast_photo_cap(msg, state):
...
async def p_bcast_text(msg, state):
    d = await state.get_data(); await state.clear()
    txt     = msg.text.strip()
    target  = d.get("bcast_target","all")
    targets = VIP_USERS if target=="vip" else ALLOWED_USERS
...
async def cb_a_addadm(cb, state):
...
async def p_addadm_id(msg, state):
    try: uid = int(msg.text.strip())
    except: await msg.answer("  ✗ Invalid ID."); return
    await state.update_data(target=uid)
