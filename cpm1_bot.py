...
class CPMNuker:
    ...
    async def unlock_equip_male(self, uid):
        await self.load(uid)
        td    = self.get_token_data(uid)
        email = td.get("email") if td else None
        d     = deepcopy(self.get_record(uid,email))
        if not d or not d.get("Name"):
            return {"ok":False,"message":"Could not load account data."}
        base = d.get("personEquipmentsMale") if isinstance(d.get("personEquipmentsMale"), dict) else {}
        def merge(k, ids):
            old = base.get(k) or []
            if not isinstance(old, list):
                old = []
            return sorted(set(int(x) for x in old if isinstance(x, (int, float))) | set(ids))
        d["personEquipmentsMale"] = {
            "Gender": 0,
            "hair": merge("hair", list(range(0, 25)) + [100]),
            "face": merge("face", [0, 1, 2, 100]),
            "beard": merge("beard", list(range(0, 25)) + [100]),
            "cap": merge("cap", list(range(0, 70)) + [100]),
            "mask": merge("mask", list(range(0, 15)) + [100]),
            "top": merge("top", list(range(0, 120))),
            "gloves": merge("gloves", list(range(0, 10)) + [100]),
            "bag": merge("bag", list(range(0, 50))),
            "pants": merge("pants", list(range(0, 40))),
            "shoes": merge("shoes", list(range(0, 40))),
            "glasses": merge("glasses", list(range(0, 15)) + [100]),
            "SelectedEquipments": base.get("SelectedEquipments") or [-1, 10, 19, 41, 100, 4, 20, 9, 22, 21, 74],
        }
        return await self._save(uid,d)

    async def unlock_equip_female(self, uid):
        await self.load(uid)
        td    = self.get_token_data(uid)
        email = td.get("email") if td else None
        d     = deepcopy(self.get_record(uid,email))
        if not d or not d.get("Name"):
            return {"ok":False,"message":"Could not load account data."}
        base = d.get("personEquipmentsFemale") if isinstance(d.get("personEquipmentsFemale"), dict) else {}
        def merge(k, ids):
            old = base.get(k) or []
            if not isinstance(old, list):
                old = []
        ...
        d["personEquipmentsFemale"] = {
            "Gender": 1,
            "hair": merge("hair", list(range(0, 20)) + [100]),
            "face": merge("face", [0, 1, 2]),
            "beard": [],
            "cap": merge("cap", list(range(0, 45)) + [100]),
            "mask": merge("mask", list(range(0, 12)) + [100]),
            "top": merge("top", list(range(0, 90))),
            "gloves": merge("gloves", list(range(0, 8)) + [100]),
            "bag": merge("bag", list(range(0, 20))),
            "pants": merge("pants", list(range(0, 25))),
            "shoes": merge("shoes", list(range(0, 25))),
            "glasses": merge("glasses", list(range(0, 12)) + [100]),
            "SelectedEquipments": base.get("SelectedEquipments") or [0, 0, -1, -1, -1, -1, -1, -1, 0, -1, -1],
        }
        return await self._save(uid,d)
...
def cpm1_clone_account(source_email, source_pass, target_email, target_pass):
    ...
    for idx, car in enumerate(cars):
        if not isinstance(car, dict):
            continue
        if cpm1_clone_car(target_token, car, target_uid, token_source=source_token):
            success_count += 1
        else:
            fail_count += 1
        time.sleep(0.6)
    ...
    if success_count == total_cars:
        ...
        result_data["error"] = "All cars failed. Open target in game once / garage full."
...
def cpm1_inject_car(email, password, car_id):
    try:
        ...
        try:
            ...
            return status == 200 and result.get("result") == 1
        except Exception:
            return False
    except Exception:
        return False
...
class K:
    ...
    def feat():
        return InlineKeyboardMarkup(inline_keyboard=[
            ...
            [InlineKeyboardButton(text="🚨 Siren",   callback_data="f_siren"),
             InlineKeyboardButton(text="👔 M Equip", callback_data="f_equip_m")],
            [InlineKeyboardButton(text="👗 F Equip", callback_data="f_equip_f"),
             InlineKeyboardButton(text="🚗 Unlock Cars", callback_data="f_unlock_cars")],
    ...
    def sett():
        return InlineKeyboardMarkup(inline_keyboard=[
            ...
            [InlineKeyboardButton(text="🧬 Clone Account", callback_data="act_clone")],
            [InlineKeyboardButton(text="◂ Back", callback_data="back_home")],
        ])
...
FEAT_MAP = {
    ...
    "f_equip_m":("👔 Male Equip",    nuker.unlock_equip_male),
    "f_equip_f":("👗 Female Equip",  nuker.unlock_equip_female),
}
...
# ═══════════════════════════════════════════
#  🧬 CLONE ACCOUNT & 🚗 UNLOCK CARS
# ═══════════════════════════════════════════

@rt.callback_query(F.data == "act_clone")
async def cb_act_clone(cb: CallbackQuery, state: FSMContext):
...
async def p_clone_tp(msg: Message, state: FSMContext):
    ...
    await state.clear()
    m = await msg.answer(f"{B}\n  🧬 Cloning cars...\n{B}")
    ok, data = await asyncio.to_thread(
        cpm1_clone_account, d.get("se"), d.get("sp"), d.get("te"), msg.text.strip()
    )
    if ok is True:
        txt = f"✅ Cloned {data['success']}/{data['total']} cars"
    elif ok == "partial":
        txt = f"⚠️ Partial {data['success']}/{data['total']} (fail {data['fail']})"
    else:
        txt = f"❌ {data.get('error', 'failed')} ({data.get('success', 0)}/{data.get('total', 0)})"
    await m.edit_text(f"{B}\n  {txt}\n{B}", reply_markup=K.back_home())

@rt.callback_query(F.data == "f_unlock_cars")
async def cb_unlock_cars(cb: CallbackQuery, state: FSMContext):
    await state.set_state(SUnlockCars.email)
    await cb.message.edit_text(
        f"{B}\n  🚗  𝗨𝗡𝗟𝗢𝗖𝗞 𝗖𝗔𝗥𝗦\n{B}\n\n  Send account email:",
        reply_markup=K.cancel())
    await cb.answer()

@rt.message(SUnlockCars.email)
async def p_uc_email(msg: Message, state: FSMContext):
    await state.update_data(ue=msg.text.strip())
    await state.set_state(SUnlockCars.password)
    await msg.answer("  Send password:")

@rt.message(SUnlockCars.password)
async def p_uc_pass(msg: Message, state: FSMContext):
    d = await state.get_data()
    await state.clear()
    email, password = d.get("ue"), msg.text.strip()
    m = await msg.answer(f"{B}\n  🚗 Unlocking cars...\n{B}")
    ok, info = await asyncio.to_thread(cpm1_unlock_cars_on_account, email, password, list(range(1, 51)))
    if isinstance(info, dict):
        if info.get("error"):
            txt = f"❌ {info['error']}"
        else:
            txt = f"{'✅' if ok else '❌'} {info.get('success', 0)}/{info.get('total', 0)} (fail {info.get('fail', 0)})"
    else:
        txt = str(info)
    await m.edit_text(f"{B}\n  {txt}\n{B}", reply_markup=K.back_home())
